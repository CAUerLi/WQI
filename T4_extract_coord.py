...
def main():
    ...
    Inputpath = 'MetaData'
    Outputpath = 'MetaData/unique_sites_coordinate'
    infilename = 'meta_TN_GRQA.xlsx'
    Syear = 1978
    Eyear = 2017
    Numyear = Eyear - Syear + 1 # 40 years span
    resolution = 0.0833333333333333 # 5 arc-minutes
    OriData = pd.read_excel(os.path.join(Inputpath, infilename), engine='openpyxl',header=0, index_col=0)  # nrows=500
    ...
    statisticsData(TarData, Outputpath, 'unique_sites_coordinate', 'coordinate')
...
def statisticsData(data, filepath, filename, txtfilename):
    ...
    lat_lon_data = unique_sites_grouped[['lon_wgs84', 'lat_wgs84']]
